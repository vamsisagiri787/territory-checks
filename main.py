...
#  --------------------
#  • Week window:
#       - Default: previous Sun 00:00:00 → this Sun 00:00:00 (UTC, exclusive)
...
# Where to put files in GCS (roots only; subfolders are derived from week)
...
gv_USER_AGENT   = "TerritoryChecks/3.0"
gv_HTTP_TIMEOUT = 60
gv_MAX_RETRIES  = 5
...
def forwarded_broker_from_bodypreview(lv_body_preview: str) -> Optional[str]:
    """
    Try to detect broker from a block of text (subject/body preview or full body).

    We first look for a literal "From: someone <email@domain>" pattern, and if
    that fails we fall back to running the subject keyword regexes.
    """
...
def delete_week_slice(
    ...
    gv_LOG.info(
        "Deleting existing week slice from %s for run_date_from=%s, run_date_to=%s",
        table_id,
        run_date_from,
        run_date_to,
    )
    ...
    res = job.result()
    gv_LOG.info(
        "Deleted %s rows from %s for the given week slice.",
        res.num_dml_affected_rows,
        table_id,
    )
...
def join_prefix(base_prefix: str, *parts: str) -> str:
    """
    Join a base GCS prefix with one or more folder parts, avoiding
...
def run(lv_override_week_end_str: Optional[str] = None) -> dict:
    ...
    for lv_folder in lv_client.list_all_folders():
        ...
        for lv_msg in lv_client.fetch_messages_in_folder(
            ...
            # Full text (for broker + territory); fetched lazily
            ...
            if lv_chosen_broker == "Others":
                try:
                    ...
                    lv_broker_full = forwarded_broker_from_bodypreview(lv_full_text)
                    if lv_broker_full:
                        lv_chosen_broker = lv_broker_full
                except Exception as lv_e:
                    gv_LOG.debug(
                        "Full body fetch for broker failed for %s: %s",
                        lv_msg.get("id"),
                        lv_e,
                    )
            ...
            if not lv_terr:
                if lv_full_text is None:
                    try:
                        lv_body_html = lv_client.fetch_full_body(lv_msg["id"])
                        lv_full_text = _html_to_text(lv_body_html)
                    except Exception as lv_e:
                        gv_LOG.debug(
                            "Full body fetch failed for territory %s: %s",
                            lv_msg.get("id"),
                            lv_e,
                        )
                        lv_full_text = None
...
def main() -> None:
    ...
    lv_parser.add_argument(
        ...
        required=False,
        help=(
            "Optional override for week end date in UTC, "
            "format YYYY-MM-DD (expected to be a Sunday). "
            "If omitted, the last completed week based on 'now' is used."
        ),
    )
    ...
    if lv_args.week_end:
        gv_LOG.info(
            "Starting Territory Checks ETL job for explicit week_end=%s",
            lv_args.week_end,
        )
    else:
        gv_LOG.info(
            "Starting Territory Checks ETL job for last completed week (no override)."
        )
